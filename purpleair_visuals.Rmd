...
p1 <- ggplot(purpdf, aes(time_utc,y)) +
...
p2 <- ggplot(purpdf, aes(time_utc,y)) +
...
p3 <- ggplot(purpdf, aes(time_utc,y)) +
...
# bins Facetted 
ggplot(f,aes(time_utc, Counts, color = Particle_Size)) +
...
hpurpdf <- purpdf %>%
  mutate(hour = hour(time_utc), date = as_date(time_utc)
  ) %>%
